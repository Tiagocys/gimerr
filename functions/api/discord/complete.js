...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const result = await verifyDiscordState(env, body.result);

    if (result.type !== "discord_connection" || result.userId !== auth.user.id || !result.discord?.id) {
      return jsonResponse({ error: "Resultado Discord inválido." }, { status: 400 });
    }
    ...
    const response = await fetch(`${supabaseRestUrl}/profile_platform_links?on_conflict=profile_id,platform`, {
      ...
      body: JSON.stringify({
        ...
        platform: "discord",
        handle: result.discord.handle,
        profile_url: result.discord.profileUrl,
        external_user_id: result.discord.id,
        is_public: true,
    ...
    return jsonResponse({
      error: error?.message || "Não foi possível concluir a conexão Discord.",
    }, { status: 500 });
  }
}
